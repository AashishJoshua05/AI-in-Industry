...
data = pd.DataFrame({
    'Process': ['A', 'B', 'C', 'D'],
    'Arrival Time': [0, 10, 15, 20],
    'Estimated Treatment Time': [30, 20, 40, 15],
    'Priority': [3, 5, 2, 4]
})
...
data_fcfs['Waiting Time'] = [0] + [max(0, data_fcfs['Estimated Treatment Time'].values[i-1] - data_fcfs['Arrival Time'].values[i]) for i in range(1, len(data_fcfs))]
data_fcfs['Turnaround Time'] = data_fcfs['Waiting Time'] + data_fcfs['Estimated Treatment Time']
...
data_sjf = data.sort_values(by='Estimated Treatment Time')
data_sjf['Waiting Time'] = [0] + [max(0, data_sjf['Estimated Treatment Time'].values[i-1] - data_sjf['Arrival Time'].values[i]) for i in range(1, len(data_sjf))]
data_sjf['Turnaround Time'] = data_sjf['Waiting Time'] + data_sjf['Estimated Treatment Time']
...
data_ps['Waiting Time'] = [0] + [sum(data_ps['Estimated Treatment Time'][:i]) - data_ps['Arrival Time'].values[i] for i in range(1, len(data_ps))]
data_ps['Turnaround Time'] = data_ps['Waiting Time'] + data_ps['Estimated Treatment Time']
...
while data_rr['Estimated Treatment Time'].sum() > 0:
    for i in range(len(data_rr)):
        if data_rr['Estimated Treatment Time'][i] > 0:
            if data_rr['Estimated Treatment Time'][i] > time_quantum:
                data_rr['Waiting Time'][i] += sum(data_rr['Estimated Treatment Time'][:i]) - data_rr['Arrival Time'][i]
                data_rr['Estimated Treatment Time'][i] -= time_quantum
            else:
                data_rr['Waiting Time'][i] += sum(data_rr['Estimated Treatment Time'][:i]) - data_rr['Arrival Time'][i]
                data_rr['Estimated Treatment Time'][i] = 0
